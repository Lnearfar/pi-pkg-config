...
test("project sources shorten to a cwd-relative path while global sources use the home path", async () => {
	...
	try {
		...
		assert.equal(catalog.project.find((resource) => resource.name === "project-skill")?.groupLabel, "Project .agents/skills");
		assert.equal(catalog.project.find((resource) => resource.name === "global-skill")?.groupLabel, "Global ~/.pi/agent/skills");
	} finally {
